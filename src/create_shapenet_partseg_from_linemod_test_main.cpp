...
//    ss.push_back("02");
    ...
    for(int ss_idx = 4; ss_idx < ss.size(); ss_idx++)
    {
...
//        cout << rgbPath[0] << endl;
        ...
        vector<Single> single(filesPath.size());
        for(int gtPosIdx = 0; gtPosIdx < filesPath.size(); gtPosIdx++)
        {
            stringstream sss;
            sss << gtPosIdx;
            doc[sss.str()][0] >> single[gtPosIdx];
        }

        /// load model
        string model_path = modelBasePath + "obj_" + ss[ss_idx] + ".ply";
        Cloud6D::Ptr model(new Cloud6D());
        pcl::io::loadPLYFile(model_path, *model);
...
//                    cout << "pb,pg,pr:" << p->b << "," << p->g << "," << p->r << endl;
            ...
            matrix4f.block<3,3>(0,0) = single[rgb_index].matrix3f;
            matrix4f.block<3,1>(0,3) = single[rgb_index].vector3f;
            Cloud6D::Ptr tfModel(new Cloud6D());
            pcl::transformPointCloud(*model, *tfModel, matrix4f);
            // set search radius
            float Radius = 10;
            /// search the ture seg
            ...
            {
                if(kdtree.radiusSearch(new_cloud->points[i], Radius, pointIdxRadiusSearch, pointRadiusSquaredDistance)>0)
                {
                    labels[i] = stoi(ss[ss_idx]);
                }
            }
            /// save data
//            for(size_t i = 0; i < new_cloud->points.size(); ++i)
...
//            }
            ...
            cout << ss[ss_idx] << ": " << rgb_index_n_s.substr(
                    rgb_index_n_s.size()-4, rgb_index_n_s.size()) << ".pts" << endl;
            TotalIndex++;
        }
